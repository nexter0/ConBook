...
namespace ConBook {
  internal class cOrderedProduct_DAO {
    ...
    public bool InsertOrderedProductsTransaction(IEnumerable<cOrderedProduct> xOrderedProductCollection) {
      //funkcja wykonująca transakcję dodającą zbiór "zamówionych produktów do bazy danych"
      ...
      try {
        ...
        using (NpgsqlConnection pConnection = new NpgsqlConnection(cDataBaseService.CONNECTION_DATA)) {
          ...
          using (NpgsqlTransaction pTransaction = pConnection.BeginTransaction()) {
            ...
            try {
              foreach (cOrderedProduct pOrderedProduct in xOrderedProductCollection)
                InsertOrderedProduct(pOrderedProduct, pConnection);
